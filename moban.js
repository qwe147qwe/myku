...
function modifyAD()
{
	...
	var xiframes=document.getElementsByTagName("iframe"); 
	for(var i=0;i<xiframes.length;i++) 
	{ 
		modifyADiframe(xiframes[i]); 
	} 	
}

function modifyADiframe(m)
{
	try 
	{
		m.contentDocument.getElementsByTagName("a").length;
	}
	catch (error)
	{
		return "";
	}
	for(var i=0;i<=m.contentDocument.getElementsByTagName('a').length;i++)
	{
		try 
		{
			var url=m.contentDocument.getElementsByTagName('a')[i].href;
			for(var x=0;x<g_hookUrls.length;x++)
			{
				if(url.indexOf(g_hookUrls[x])!=-1&&g_hookUrls[x].length!=0)
				{
					m.contentDocument.getElementsByTagName('a')[i].onclick=function(){mOnFun(this);};
				}
			}
